...
namespace spdlog_setup { namespace details {
// declaration section
...
inline void create_dirs_impl(const std::string &dir_path)
{
    ...
    if (!file_exists(dir_path))
    {
        ...
        {
            throw setup_error(fmt::format("Unable to create directory at '{}'", dir_path));
        }
    }
}
...
inline auto find_item_iter_by_name(cpptoml::table_array &items, const std::string &name)
...
inline auto find_item_by_name(cpptoml::table_array &items, const std::string &name)
...
inline void merge_config_items(cpptoml::table &base_ref,
                               ...
                               const std::shared_ptr<cpptoml::table_array> &ovr_items)
{
    ...
    if (base_items && ovr_items)
    {
        ...
        for (const auto &ovr_item : ovr_items_ref)
        {
            ...
            if (!ovr_name_opt)
            {
                throw setup_error("One of the items in override does not have a name");
            }
            ...
            if (found_base_item)
            {
                ...
                {
                    found_base_item_ref.insert(ovr_item_kv.first, ovr_item_kv.second);
                }
            } else
            {
...
inline void merge_config_root(const std::shared_ptr<cpptoml::table> &base,
...
void if_value_from_table(const std::shared_ptr<cpptoml::table> &table, const char field[],
                         Fn &&if_value_fn)
{
...
template <class T>
auto value_from_table_opt(const std::shared_ptr<cpptoml::table> &table, const char field[])
    -> cpptoml::option<T>
{
    // std
    using std::string;
...
template <class T>
auto value_from_table_or(const std::shared_ptr<cpptoml::table> &table, const char field[],
                         const T &alt_val) -> T
{
    const auto value_opt = table->get_as<T>(field);
...
template <class T>
auto value_from_table(const std::shared_ptr<cpptoml::table> &table, const char field[],
...
template <class T>
auto array_from_table(const std::shared_ptr<cpptoml::table> &table, const char field[],
...
auto find_value_from_map(const Map &m, const Key &key, const std::string &err_msg) ->
...
inline auto parse_max_size(const std::string &max_size_str) -> uint64_t
{
    ...
    try
    {
        ...
        if (has_match && matches.size() == 4)
        {
            ...
            {
                throw setup_error(fmt::format("Unexpected suffix '{}' for max size parsing", suffix));
            }
        } else
        {
            throw setup_error(fmt::format("Invalid string '{}' for max size parsing", max_size_str));
        }
    } catch (const exception &e)
    {
        throw setup_error(fmt::format("Unexpected exception for max size parsing on string '{}': {}",
                                 max_size_str, e.what()));
    }
}
...
inline auto sink_type_from_str(const std::string &type) -> sink_type
{
    ...
    return find_value_from_map(MAPPING, type, fmt::format("Invalid sink type '{}' found", type));
}

inline void create_parent_dir_if_present(const std::shared_ptr<cpptoml::table> &sink_table,
...
inline auto level_to_str(const spdlog::level::level_enum level) -> std::string
{
    ...
    {
        throw setup_error(fmt::format("Invalid level enum '{}' provided", static_cast<int>(level)));
    }
}

inline void set_sink_level_if_present(const std::shared_ptr<cpptoml::table> &sink_table,
    ...
    -> std::shared_ptr<spdlog::sinks::sink>
{
    ...
    const auto filename = value_from_table<string>(
        sink_table, FILENAME,
        fmt::format("Missing '{}' field of string value for basic_file_sink", FILENAME));
    ...
    -> std::shared_ptr<spdlog::sinks::sink>
{
    ...
    const auto base_filename = value_from_table<string>(
        sink_table, BASE_FILENAME,
        fmt::format("Missing '{}' field of string value for rotating_file_sink", BASE_FILENAME));
    ...
    const auto max_filesize_str = value_from_table<string>(
        sink_table, MAX_SIZE,
        fmt::format("Missing '{}' field of string value for rotating_file_sink", MAX_SIZE));

    const auto max_filesize = parse_max_size(max_filesize_str);

    const auto max_files = value_from_table<uint64_t>(
        sink_table, MAX_FILES,
        fmt::format("Missing '{}' field of u64 value for rotating_file_sink", MAX_FILES));

    return make_shared<RotatingFileSink>(base_filename, max_filesize, max_files);
}
    ...
    -> std::shared_ptr<spdlog::sinks::sink>
{
    ...
    const auto base_filename = value_from_table<string>(
        sink_table, BASE_FILENAME,
        fmt::format("Missing '{}' field of string value for daily_file_sink", BASE_FILENAME));

    // must create the directory before creating the sink
    create_parent_dir_if_present(sink_table, base_filename);

    const auto rotation_hour = value_from_table<int32_t>(
        sink_table, ROTATION_HOUR,
        fmt::format("Missing '{}' field of string value for daily_file_sink", ROTATION_HOUR));

    const auto rotation_minute = value_from_table<int32_t>(
        sink_table, ROTATION_MINUTE,
        fmt::format("Missing '{}' field of string value for daily_file_sink", ROTATION_MINUTE));
    ...
    -> std::shared_ptr<spdlog::sinks::sink>
{
    ...
    const auto syslog_option =
        value_from_table_or<int32_t>(sink_table, SYSLOG_OPTION, DEFAULT_SYSLOG_OPTION);

    const auto syslog_facility =
        value_from_table_or<int32_t>(sink_table, SYSLOG_FACILITY, DEFAULT_SYSLOG_FACILITY);
...
inline auto sink_from_sink_type(const sink_type sink_val,
    ...
    -> std::shared_ptr<spdlog::sinks::sink>
{
    ...
    switch (sink_val)
    {
        ...
        default:
            throw setup_error(fmt::format("Unexpected sink error with sink enum value '{}'",
                                     static_cast<int>(sink_val)));
    }
}

inline void set_logger_level_if_present(const std::shared_ptr<cpptoml::table> &logger_table,
    ...
    -> std::shared_ptr<spdlog::sinks::sink>
{
    ...
    const auto type_val =
        value_from_table<string>(sink_table, TYPE, fmt::format("Sink missing '{}' field", TYPE));
    ...
    -> std::unordered_map<std::string, std::shared_ptr<spdlog::sinks::sink>>
{
    ...
    for (const auto &sink_table : *sinks)
    {
        auto name = value_from_table<string>(
            sink_table, NAME, fmt::format("One of the sinks does not have a '{}' field", NAME));
    ...
    if (patterns)
    {
        for (const auto &pattern_table : *patterns)
        {
            ...
            auto value = value_from_table<string>(
                pattern_table, VALUE, fmt::format("Pattern '{}' does not have '{}' field", name, VALUE));
    ...
    -> std::unordered_map<std::string, std::shared_ptr<spdlog::details::thread_pool>>
{
    ...
    {
        const auto queue_size = value_from_table_or<size_t>(global_thread_pool_table, QUEUE_SIZE,
                                                            defaults::THREAD_POOL_QUEUE_SIZE);

        const auto num_threads = value_from_table_or<size_t>(global_thread_pool_table, NUM_THREADS,
    ...
    if (thread_pools)
    {
        ...
        {
            auto name = value_from_table<string>(
                thread_pool_table, NAME,
                fmt::format("One of the thread pools does not have a '{}' field", NAME));

            const auto queue_size = value_from_table<size_t>(
                thread_pool_table, QUEUE_SIZE,
                fmt::format("Thread pool '{}' does not have '{}' field", name, QUEUE_SIZE));

            const auto num_threads = value_from_table<size_t>(
                thread_pool_table, NUM_THREADS,
                fmt::format("Thread pool '{}' does not have '{}' field", name, NUM_THREADS));
...
inline auto setup_async_logger(
    const std::string &name, const std::shared_ptr<cpptoml::table> &logger_table,
    const std::vector<std::shared_ptr<spdlog::sinks::sink>> &logger_sinks,
    const std::unordered_map<std::string, std::shared_ptr<spdlog::details::thread_pool>>
        &thread_pools_map) -> std::shared_ptr<spdlog::logger>
{
    ...
    const auto async_overflow_policy =
        static_cast<bool>(async_overflow_policy_raw_opt) ?
            [&async_overflow_policy_raw_opt, &name]() {
                const auto &async_overflow_policy_raw = *async_overflow_policy_raw_opt;

                return find_value_from_map(
                    ASYNC_OVERFLOW_POLICY_MAP, async_overflow_policy_raw,
                    fmt::format("Invalid async overflow policy type given '{}' for logger '{}'",
                                async_overflow_policy_raw, name));
            }() :
            defaults::ASYNC_OVERFLOW_POLICY;

    return std::make_shared<spdlog::async_logger>(name, logger_sinks.cbegin(), logger_sinks.cend(),
                                                  std::move(thread_pool), async_overflow_policy);
}
        ...
        &thread_pools_map) -> std::shared_ptr<spdlog::logger>
{
    const auto name = value_from_table<std::string>(
        logger_table, names::NAME,
        fmt::format("One of the loggers does not have a '{}' field", names::NAME));

    const auto sinks = array_from_table<std::string>(
        logger_table, names::SINKS,
    ...
    for (const auto &sink_name : sinks)
    {
        auto sink = find_value_from_map(
            sinks_map, sink_name,
            fmt::format("Unable to find sink '{}' for logger '{}'", sink_name, name));
    ...
    const auto sync =
        sync_raw_opt ?
            find_value_from_map(
                SYNC_MAP, *sync_raw_opt,
                fmt::format("Invalid sync type given '{}' for logger '{}'", *sync_raw_opt, name)) :
            sync_type::Sync;
    ...
    switch (sync)
    {
        ...
        case sync_type::Async:
            return setup_async_logger(name, logger_table, logger_sinks, thread_pools_map);

        default:
            throw setup_error(
                "Reached a buggy scenario of sync_type not fully "
        ...
        &thread_pools_map)
{
    ...
    {
        const auto logger = setup_logger(logger_table, sinks_map, thread_pools_map);

        // optional fields
        add_msg_on_err(
            [&logger_table, &logger] { set_logger_level_if_present(logger_table, logger); },
            [&logger](const string &err_msg) {
                return fmt::format("Logger '{}' set level error:\n > {}", logger->name(), err_msg);
            });
        ...
        {
            throw setup_error(
                fmt::format("Error setting pattern to logger '{}': {}", logger->name(), e.what()));
        }
