 ...
 * @throw setup_error
 */
auto from_file_and_override(const std::string &base_toml_path,
                            const std::string &override_toml_path) -> bool;
 ...
 * @throw setup_error
 */
void save_logger_to_file(const std::shared_ptr<spdlog::logger> &logger,
                         const std::string &toml_path, const bool overwrite = false);
                                                 ...
                                                 const Ps &... ps) -> bool
{
    ...
    try
    {
        ...
        if (has_override)
        {
            ...
            details::read_template_file_into_stringstream(override_toml_ss, override_pre_toml_path,
                                                          ps...);
...
inline auto from_file_and_override(const std::string &base_toml_path,
                                   const std::string &override_toml_path) -> bool
{
    // std
    using std::exception;
...
inline void save_logger_to_file(const std::shared_ptr<spdlog::logger> &logger,
                                const std::string &toml_path, const bool overwrite)
{
...
inline auto delete_logger_in_file(const std::string &logger_name, const std::string &toml_path)
    -> bool
{
    ...
    try
    {
        ...
        if (!config)
        {
            throw setup_error(fmt::format("Unable to parse file at '{}' for deleting logger '{}'",
                                     toml_path, logger_name));
        }
        ...
        {
            throw setup_error(
                fmt::format("Unable to find any logger table array for file at '{}'", toml_path));
        }
